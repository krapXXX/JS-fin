    ...
    document.body.addEventListener('click', (event) => {
        if (event.target.classList.contains('buy')) {
            ...
            function checkErrorAndSetMessage(input) {
                ...
                } else if (input.name === "firstName" && input.validity.tooShort) {
                    input.setCustomValidity("First Name must be at least 3 characters long.");
                } else if (input.name === "lastName" && input.validity.tooShort) {
                    input.setCustomValidity("Last Name must be at least 6 characters long.");
                } else if (input.name === "city" && input.validity.tooShort) {
                    input.setCustomValidity("City must be at least 5 characters long.");
                } else if (input.name === "postOffice" && input.validity.valueMissing) {
                    input.setCustomValidity("Post Office cannot be empty.");
                } else if (input.name === "paymentOption" && input.validity.valueMissing) {
                    input.setCustomValidity("Please select a payment option.");
                }
            }
